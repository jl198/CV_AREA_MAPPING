...
filename = r'/home/jake/cv_area_mapping/images/EMPTY_FRAME_MULTIPLE_SHOOTER_OVERCAST_ZOOMED_OUT_GX010003.png'
...
class Lane:
    ...
    def __init__(self, orig_frame):
        ...
        #     (144, 218),  # Top-left corner
        #     (0, 347),  # Bottom-left corner
        #     (767, 340),  # Bottom-right corner
        #     (596, 207)  # Top-right corner
        # ])

        self.roi_points = np.float32([
            (88, 247),  # Top-left corner
            (34, 347),  # Bottom-left corner
    ...
    def get_line_markings(self, frame=None):
        ...
        _, sxbinary = edge.threshold(hls[:, :, 1], thresh=(100, 255), thresh_type=cv2.THRESH_BINARY)
        cv2.imshow('sxbinary', sxbinary)
        sxbinary = edge.blur_gaussian(sxbinary, ksize=3)  # Reduce noise
        cv2.imshow('hls', hls)

        cv2.imshow('sxbinary_blur', sxbinary)
        cv2.waitKey()
        ...
        cv2.imshow('s_binary_sat', s_binary)
    ...
    def perspective_transform(self, frame=None, plot=False):
        ...
        self.warped_frame = cv2.warpPerspective(frame, self.transformation_matrix, self.orig_image_size,
...
def main():
    ...
    original_frame = cv2.resize(original_frame, None, fx=.2, fy=.2, interpolation=cv2.INTER_AREA)
